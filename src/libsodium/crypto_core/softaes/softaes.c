...
#    if (defined(__clang__) || (defined(__GNUC__) && __GNUC__ >= 12)) &&      \
        ...
        !defined(SOFTAES_NO_VECTOR_SBOX)
...
static void
softaes_aegis_rotate(SoftAesBlock *st, const size_t blocks)
{
    SoftAesBlock in[8];
    size_t       b;
...
#    ifdef FAVOR_PERFORMANCE
    for (b = 0; b < blocks; b++) {
        st[b] = softaes_block_encrypt(in[b], st[b]);
    }
#    else
    softaes_blocks_encrypt(st, in, st, blocks);
#    endif
}
...
softaes_aegis_rotate8_x1(SoftAesBlock st[8])
{
    softaes_aegis_rotate(st, 8);
}

void
softaes_aegis_rotate6_x1(SoftAesBlock st[6])
{
    softaes_aegis_rotate(st, 6);
}
